...
# Check Backend
echo -n "Backend API:      "
if curl -s http://localhost:7701/api/v2/stats | grep -q "total_blocks"; then
    echo -e "${GREEN}✅ OK${NC}"
    STATS=$(curl -s http://localhost:7701/api/v2/stats)
...
# Check Frontend
echo -n "Frontend:         "
if curl -s http://localhost:7401 | grep -q "html"; then
    echo -e "${GREEN}✅ OK${NC}"
else
    echo -e "${RED}❌ FAILED${NC}"
fi
